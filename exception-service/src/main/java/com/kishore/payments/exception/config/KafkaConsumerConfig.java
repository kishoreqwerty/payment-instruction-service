 ...
 * .notes/ARCHITECTURE.md §6.2: five attempts, 1s/2s/4s/8s/16s backoff, then
 * dead-letter to payments.dlq. {@code ExceptionEventConsumer} has no
 * business-failure branch of its own to catch -- unlike processing-service's
 * consumers, it never routes anything to EXCEPTION itself, it only reacts
 * to instructions that are already there -- so anything that escapes its
 * listener method uncaught is, by construction, a genuine bug or a
 * transient infrastructure failure, exactly what this retry-then-DLQ policy
 * is for.
 */
@Configuration
