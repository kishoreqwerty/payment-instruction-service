...
/** RC01: the debtor agent BIC must be a structurally valid ISO 9362 BIC. */
@Component
public class DebtorAgentBicFormatRule implements ValidationRule {

    @Override
    public Optional<FailureDetail> validate(PaymentInstructionEntity instruction) {
        String bic = instruction.getDebtorAgentBic();
        if (BicFormat.isValid(bic)) {
            return Optional.empty();
        }
        return Optional.of(new FailureDetail(
                "RC01", Repairability.REPAIRABLE, "debtorAgentBic", "Debtor agent BIC is not a valid format: " + bic));
    }
}
