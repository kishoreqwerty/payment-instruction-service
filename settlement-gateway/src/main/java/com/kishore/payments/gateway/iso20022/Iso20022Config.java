...
/**
 * One {@link Schema} and one {@link JAXBContext} per message type: pacs.008
 * (produced, then self-validated before dispatch), pacs.002 and pacs.004
 * (consumed via the callback endpoints). Kept separate rather than one
 * combined schema/context for all three for the same reason the JAXB
 * bindings themselves live in separate packages (see the xjc configuration
 * in pom.xml).
 */
@Configuration
public class Iso20022Config {
    ...
    public JAXBContext pacs008JaxbContext() {
        return context(com.kishore.payments.gateway.iso20022.generated.pacs008.Document.class);
    }

    @Bean
    public JAXBContext pacs002JaxbContext() {
        return context(com.kishore.payments.gateway.iso20022.generated.pacs002.Document.class);
    }

    @Bean
    public JAXBContext pacs004JaxbContext() {
        return context(com.kishore.payments.gateway.iso20022.generated.pacs004.Document.class);
    }
