 ...
 * different failures. Used both ways here: outbound (the assembled pacs.008
 * must validate before it ever leaves the process -- a schema-invalid
 * message reaching the rail looks like a business failure at the rail, which
 * is miserable to diagnose from the exception queue) and inbound (a
 * pacs.002/pacs.004 callback is validated before its content is trusted).
 */
@Component
